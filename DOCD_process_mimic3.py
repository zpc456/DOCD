...
def process_HospDiagnosis(infile, multi_dx_file, encounter_dict):
    ...
    with open(infile, 'r') as f:
        ...
        for line in tqdm(csv.DictReader(f)):
            ...
            count += 1
    print('Hopsital Diagnosis without encounter_id: {}'.format(missing_eid))
    print('Hopsital Diagnosis with encounter_id: {}'.format(count))
    return encounter_dict


def process_HospProcedures(infile, multi_proc_file, encounter_dict):
    ...
    with open(infile, 'r') as f:
        ...
        for line in tqdm(csv.DictReader(f)):
            ...
            count += 1
    print('Hopsital procedures without encounter_id: {}'.format(missing_eid))
...
def get_encounter_features(encounter_dict, skip_duplicate=False, min_num_codes=1, max_num_codes=30):
    ...
    for _, enc in encounter_dict.items():
        ...
        if len(vital_signs_ints) == 0:
            default_value = vital_signs_str2int.get('-1', -1)
            vital_signs_ints = [default_value] * 70
...
def get_mimiciii_datasets(data_dir, fold=0, flag="expired"):
    ...
    if os.path.exists(cached_path):
        ...
        encounter_dict = process_HospDiagnosis(HospDiagnosis_file, multi_dx_file, encounter_dict)
        print('Processing HospProcedures.csv')
        encounter_dict = process_HospProcedures(HospProcedures_file, multi_proc_file, encounter_dict)
...
if __name__ == "__main__":
    datasets, prior_guides = get_mimiciii_datasets("data/mimic3-1.4", 50)
